...
<span class="sidenote"> <!-- possible to add "inline" -->
	...
	<span class="sidenote__note">
		<!--
		Text placed in this part of the sidetone
		will be shown as the additional information
...
```html
	...
	<div>
		<p style="color: red; font-family: Helvetica, Arial, sans-serif">
			 This is a showcase.
		</p>
	</div>
...
You can also easily lazyload the image by skipping the usual `src` part of the image and instead adding the corresponding path via an additional `data-src` attribute. Also required is an additional class with the value `lazyload`.
